...
def add(texto,c):
    print("Cantidad de "+ texto)
    cantidad = int(input("que desea ordenar: "))
    tot = c * cantidad  
    cont=+1
    l_prod.append(texto)
    l_total.append(tot)
...
def again(total,l_prod,l_total,l_cantidad,contar,caso,cantidad,c,d,e,r,beb,canbeb):
    Menu(caso)
    mediopago(total,l_cantidad,l_prod,l_total)
...
def Menu(caso):
    ...
    while caso==False:
        ...
        if(opcion_hamburguesa == "1"):
            ...
            Eliminar()
           # caso=True
        elif(opcion_hamburguesa == "SALIR" or opcion_hamburguesa == "S"):
            caso = True    
        else:
            print("Por favor eligir una opción valida\n")

def mediopago(total,l_cantidad,l_prod,l_total):  
    ...
    medio_de_pago=input("Indique una opcion: ")
    ...
    if(medio_de_pago == "1"):
        ...
        print("El recargo por pago con tarjeta es de: ", Valor_Recargo )
        des=input("Continuar con el proceso de pago de tarjeta? S/N: ").upper()
        if (des=="S"):
            print("\n ---EL PEDIDO REALIZADO ES---\n ------------------------------ \n ------------------------------")
            for i in range(len(l_prod)):
                print(i+1,l_cantidad[i], l_prod[i], l_total[i])
            print("------------------------------ \n ------------------------------")
            print("El total sin recargo:     ",total)
            print("Recargo:                  ",Valor_Recargo)
            print("------------------------------")
            print("total a pagar             ",total+Valor_Recargo)
            acep=input("Desea impimir el recibo? S/N").upper()
            if (acep=="S"):
                print("Imprimiendo.......")
...
while sw==True:
    conti=input("Seleccione una opcion:\n 1. Nuevo cliente\n 2. Cerrar proceso \n Ingrse la opcion: ")
    if (conti=="1"):
        total=0
        l_prod = []
        l_total = []
        ...
        beb=[]
        canbeb=[]
        again(total,l_prod,l_total,l_cantidad,contar,caso,cantidad,c,d,e,r,beb,canbeb)
    elif(conti=="2"):
        sw=False
print("Hasta luego....")